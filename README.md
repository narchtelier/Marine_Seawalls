...
- **10x10 Interactive SOM 3D Morphospace Grid**: Generates a 100-cell latent space manifold matrix rendering 100 live 3D procedural geometries. Includes interactive color-coded heatmaps for wave dissipation ($\tau_{\text{diss}}$), recruitment score ($\sigma_{\text{rec}}$), porosity ($\Phi$), and eco-index ($J_{\text{eco}}$), allowing researchers to inspect and select optimized morphospace interpolations with a single click.
